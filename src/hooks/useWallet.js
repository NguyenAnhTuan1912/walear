...
import { clientForChain, getTxnParams } from "../utils/algod";
import { getAccountInfor, mation } from "../utils/account";
...
export function useWallet() {
  ...
  return {
    ...
    async signAndSubmitTransaction({ signer, data }) {
      try {
        // Generate params
        const params = await getTxnParams(CHAIN_TYPES.TESTNET.name);
    ...
    async executeSmartContract({ signer, method, args }) {
      try {
        const sp = await getTxnParams();
        const commonParams = {
          appId: Number(import.meta.env.VITE_APP_INDEX),
          sender: signer,
          suggestedParams: sp,
          signer: signer,
        };
        ...
        comp.buildGroup();
        const result = await comp.execute(
        ...
        console.error("Execute Smart Contract:", error);
      }
    },
